...
def build_vectorizer(max_tokens: int, max_len: int) -> layers.TextVectorization:
    """Create (but do not adapt) the text vectorization layer."""
...
def build_model(vectorizer: layers.TextVectorization, max_tokens: int) -> models.Model:
    """
    Wire an already-adapted vectorizer into the model so the vocabulary is
    frozen at build time. Uses clipnorm=1.0 to prevent NaN gradients caused
    by pathological HTML (huge pages, binary content, etc.).
    Architecture: TextVectorization -> Embedding -> GlobalAveragePooling -> Dense
    """
...
def main():
    ...
    vectorizer = build_vectorizer(args.max_tokens, args.max_len)
    vectorizer.adapt(tf.constant(texts_0))
    model = build_model(vectorizer, args.max_tokens)
    model.summary()
    ...
    # ── Save final model (.keras format required by Keras 3) ─────────────────
    final_path = output_dir / "model_final.keras"
    print(f"\nSaving final model to {final_path} ...")

    # Wrap vectorizer + model into a single end-to-end model before saving
    # so inference only needs raw HTML strings as input.
